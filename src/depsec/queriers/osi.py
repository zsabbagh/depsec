...
class OSIQuerier:
    ...
    def query_package(self, package: str, platform: str = "pypi"):
        ...
        if self.__query_failed(response):
            logger.error(
                f"Error querying OSI for '{platform}' package '{package}': {response.status_code}: {response.text}"
            )
            return None
        return response.json()

    def query_dependencies(
        self, package: str, version: str = None, platform: str = "pypi"
    ):
        """
        ...
        if version is None:
            ...
            for i in range(-1, -len(versions), -1):
                ...
                if version.get("isDefault", False):
                    ...
                    if version is None:
                        logger.error(
                            f"No version found for '{platform}' package '{package}' when searching for default version"
                        )
                        return None
                    break
        ...
        if self.__query_failed(response):
            logger.error(
                f"Error querying OSI for '{platform}' package '{package}' dependencies: {response.status_code}: {response.text}"
            )
            return None
        return response.json()
