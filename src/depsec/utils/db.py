...
from depsec.utils.tools import version_in_range
...
def is_applicable(release: Release | str | dict, applicability: dict | list) -> bool:
    ...
    version = (
        ...
        if isinstance(release, Release)
            ...
            semver.parse(release)
            if isinstance(release, str)
            else semver.parse(release["version"])
    ...
    if version is None:
        logger.error(
            f"Could not parse version from release type: {type(release).__name__}"
        )
        return False
    for app in apps:
        version_start = app.get("version_start")
        version_start = (
            semver.parse(version_start.strip("."))
            if version_start is not None
            else version_start
        )
        version_end = app.get("version_end")
        version_end = (
            semver.parse(version_end.strip("."))
        ...
        if version_start is not None or version_end is not None:
            if version_in_range(
                version, version_start, version_end, exclude_start, exclude_end
            ):
                applies = True
                break
...
def reliable_published_date(release: Release) -> datetime.datetime | None:
    ...
    return (
        release
        and ((release.osi_verified and release.published_at) or release.commit_at)
    ) or None
    ...
    if versions != set():
        ...
        relselect = sorted(
            [
                ...
                if version_in_range(rel.version, previous_version)
            ],
        ...
        for rel in relselect:
            ...
            start_rel = Release.get_or_none(
                Release.project == project, Release.version == previous_version
            )
            end_rel = Release.get_or_none(
                Release.project == project, Release.version == rel.version
            )
...
class CWE:
    def get(
        cwe_id: cwe.Entry | str | int, to_dict: bool = True
    ) -> cwe.Entry | dict | None:
        """
    ...
    def relations(
        ...
        for relation in relations:
            logger.debug(
                f"Found relation of kind '{relation.kind}', from {relation.main.cwe_id} to {relation.other_id}"
            )
    ...
    def categories(
        entry: cwe.Entry | str | int, as_entry: bool = False, to_dict: bool = True
    ) -> cwe.Entry | dict | None:
        """
        Fetch the category of a CWE entry.
        ...
        attrs: The attributes to include in the dictionary
        """
        return CWE.relations(
            entry, kind="IsMemberOf", as_entry=as_entry, to_dict=to_dict
        )


class NVD:
    def get(entry: nvd.CVE | str, to_dict: bool = True) -> nvd.CVE | dict | None:
        """
    ...
    def cwes(
        ...
        for cw in cwes:
            ...
            if categories:
                ...
                for c in cws:
                    logger.debug(
                        f"Adding CWE {c.cwe_id} to the list, with status {c.status}"
                    )
        ...
        results = [
            res
            for res in results
            if not exclude_deprecated
            or res.status.lower() not in ("deprecated", "obsolete")
        ]
