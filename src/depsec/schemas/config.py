...
class DatabaseConfig:
    ...
    def is_set(self):
        ...
        """
        type_is_correct = type(self.__database).__name__ not in [
            "NoneType",
            "DatabaseProxy",
        ]
    ...
    def set(self, path: str, name: str = None):
        ...
        if not path.exists():
            raise ValueError(
                "Path does not exist, cannot create database to non-existent path"
            )
        ...
        logger.debug(
            f"Database set to {path}, creating tables '{', '.join(table_names) if len(table_names) > 0 else 'None'}'"
        )
